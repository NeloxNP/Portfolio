...
const ProjectCard = ({ p, index }) => {
  ...
  return (
    <article
      ...
      className="group fade-in-up relative bg-white/80 backdrop-blur border border-[#0F2A5E]/10 rounded-3xl p-6 md:p-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-500"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className={`w-10 h-10 rounded-full grid place-items-center ${a.soft} ${a.text} font-serif-display text-base`}>
            0{index + 1}
          </span>
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-[#0F2A5E]/55">{p.category}</p>
            <p className="text-[11px] text-[#0F2A5E]/45">{p.year}</p>
          </div>
        </div>
        <span className={`w-9 h-9 rounded-full grid place-items-center bg-[#0F2A5E] text-white transition-transform duration-300 ${hover ? "rotate-45" : ""}`}>
      ...
      <p className={`mt-2 text-sm md:text-base font-medium ${a.text}`}>{p.subtitle}</p>
      <p className="mt-4 text-[#0F2A5E]/75 text-[15px] leading-relaxed">{p.description}</p>

      <div className="mt-6 flex flex-wrap gap-2">
        {p.tags.map((t) => (
          <span key={t} className="text-[11px] tracking-wide px-2.5 py-1 rounded-full border border-[#0F2A5E]/15 text-[#0F2A5E]/75 bg-white/60">
            {t}
          </span>
        ))}
      </div>
      ...
      <svg viewBox="0 0 400 30" className="mt-6 w-full text-[#0F2A5E]/10">
        <path d="M0,15 C50,5 100,25 150,15 C200,5 250,25 300,15 C350,5 400,20 400,15 L400,30 L0,30 Z" fill="currentColor" />
      </svg>
    </article>
  );
};
